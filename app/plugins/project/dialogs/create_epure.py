...
class CreateEpureDialog(BaseDialog):
    def __init__(self, project_item, project_id, up_node_id, flags=None, *args, **kwargs):
        ...
        # self.selectParam = ExtendedComboBox(self)

        self.extra_param = ''
        self.extra_param_values = []
    ...
    def accept(self) -> None:
        if not self.check_name():
            ...
            return
        epure_with_props = sp.create_epure(self.ui.nameLineEdit.text(), self.folder_id,
                                           str([self.parameters[p].excel_param_name for p in self.selected_params]),
                                           str(self.oy_list),
                                           self.extra_param if self.extra_param is not None else '',
                                           str(self.extra_param_values))
