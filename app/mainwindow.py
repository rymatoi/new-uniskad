...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self._progress_details: List[str] = []
        ...
        self.progress_status_button.setAutoRaise(True)
        self.progress_status_button.setVisible(False)
        self.progress_status_button.setStyleSheet(
            "QToolButton { border: none; padding: 0 8px; color: #f1f1f1; }"
            "QToolButton:hover { text-decoration: underline; }"
        )
        self.progress_status_button.setText('Показать загрузку…')
    ...
    def show_progress_notification(self, text, details=None):
        ...
        notification.details_visibility_changed.connect(self._on_progress_details_visibility_changed)
        notification.collapse_requested.connect(self.collapse_progress_to_statusbar)
    ...
    def _reset_progress_tracking(self):
        ...
        self.progress_status_button.setText('Показать загрузку…')
        self.progress_status_button.hide()

    def _refresh_progress_details(self):
        if not self._progress_notification or not self._progress_details_visible:
            return
        if not self._progress_details:
            return
        details_text = '\n'.join(self._progress_details)
        self._progress_notification.set_details(details_text)

    def _record_progress_step(self, message: str):
        if not message:
            return
        if self._progress_details and self._progress_details[-1] == message:
            return
        self._progress_details.append(message)
        if self._progress_notification:
            self._progress_notification.enable_details(True)
        if self._progress_details_visible:
            self._refresh_progress_details()

    def _on_progress_details_visibility_changed(self, visible: bool):
        self._progress_details_visible = visible
        if visible:
            self._refresh_progress_details()

    def _on_progress_notification_closed(self, notification):
        if notification is self._progress_notification:
            self._progress_notification = None
            self._last_worker_error = None
            self._reset_progress_tracking()

    def collapse_progress_to_statusbar(self):
        if not self._progress_notification or self._progress_collapsed:
            return
        self._progress_collapsed = True
        summary = self._current_progress_message or 'Загрузка…'
        summary = summary.strip()
        if len(summary) > 40:
            summary = f"{summary[:37]}…"
        self.progress_status_button.setText(f"{summary} ▸")
        self.progress_status_button.show()
        self.status_label.setText(self._current_progress_message)
        self._progress_notification.hide()
        self.notification.request_relayout()
    ...
    def restore_progress_notification(self):
        ...
        self.progress_status_button.setText('Показать загрузку…')
        self.status_label.clear()
        self._progress_notification.show()
        self.notification.show()
        self.notification.request_relayout()
        if self._progress_details_visible:
            self._refresh_progress_details()
    ...
    def on_worker_started(self):
        ...
        if self._progress_notification is not None:
            self._record_progress_step(self._current_progress_message)
    ...
    def set_progress_bar_status(self, message):
        ...
        if self._progress_notification is not None:
            ...
            self._record_progress_step(message)
        if self._progress_collapsed:
            self.status_label.setText(message)
            summary = message.strip() if message else 'Загрузка…'
            if len(summary) > 40:
                summary = f"{summary[:37]}…"
            self.progress_status_button.setText(f"{summary} ▸")
